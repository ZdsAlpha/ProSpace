...
def Generate(Level,Height):
    ...
    if Height < r:
        r=Height
    for i in range(r):
        region.set(i,Height-i,Solid)
        if (Seed+i)%61==0:
            region.set(i,Height-i+1,Checkpoint)
        elif (Seed+i)%91==54:
            region.set(i,Height- i+1,Life)
        elif (Seed+i)%51==12:
            region.set(i,Height- i+2,DullMan)
        elif (Seed+i)%98==45:
            region.set(x,Height-i,Health0)
        elif (Seed+i)%97==76:
            region.set(x,Height-i,Boost0)
        elif (Seed+i)%96==23:
            region.set(i,Height-i,Fuel0)
    Height -= r
    #------------------
